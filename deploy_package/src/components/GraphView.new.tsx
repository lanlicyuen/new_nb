// filepath: /Users/lanlic/Desktop/vs_web/src/components/GraphView.tsx
// 网络化思考视图组件
import React, { useState, useMemo } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { Search, Filter, Share2, Info, X } from 'lucide-react';
import NetworkGraph, { GraphNode, GraphLink } from './NetworkGraph';
 ...
 */
const GraphView: React.FC<GraphViewProps> = ({ tools, className = '' }) => {
  ...
  const graphData = useMemo(() => {
    ...
    if (!filterOptions.showFeatures) {
      ...
      filteredLinks = filteredLinks.filter(link => 
        !filteredNodes.some(node => 
          node.type === 'feature' && (link.source === node.id || link.target === node.id)
        )
      );
    }
    ...
    if (!filterOptions.showTags) {
      ...
      filteredLinks = filteredLinks.filter(link => 
        !filteredNodes.some(node => 
          node.type === 'tag' && (link.source === node.id || link.target === node.id)
        )
      );
    }
